...
typedef enum {
   ...
   opPOT = 7,    /* RR     reg(r) = reg(s)^reg(t) */
   opMOD = 8,    /* RR     reg(r) = reg(s)%reg(t) */
   opRRLim = 9,   /* limit of RR opcodes */

   /* RM instructions */
   opLD = 10,      /* RM     reg(r) = mem(d+reg(s)) */
   opST = 11,      /* RM     mem(d+reg(s)) = reg(r) */
   opRMLim = 12,   /* Limit of RM opcodes */

   /* RA instructions */
   opLDA = 13,     /* RA     reg(r) = d+reg(s) */
   opLDC = 14,     /* RA     reg(r) = d ; reg(s) is ignored */
   opJLT = 15,     /* RA     if reg(r)<0 then reg(7) = d+reg(s) */
   opJLE = 16,     /* RA     if reg(r)<=0 then reg(7) = d+reg(s) */
   opJGT = 17,     /* RA     if reg(r)>0 then reg(7) = d+reg(s) */
   opJGE = 18,     /* RA     if reg(r)>=0 then reg(7) = d+reg(s) */
   opJEQ = 19,     /* RA     if reg(r)==0 then reg(7) = d+reg(s) */
   opJNE = 20,     /* RA     if reg(r)!=0 then reg(7) = d+reg(s) */
   opRALim = 21    /* Limit of RA opcodes */
   } OPCODE;
...
char opCodeTab[][5]
        = {"HALT","IN","OUT","ADD","SUB","MUL","DIV","POT","MOD","????",
            /* RR opcodes */
           "LD","ST","????", /* RM opcodes */
...
{ INSTRUCTION currentinstruction  ;
  ...
  { /* RR instructions */
    ...
    case opPOT :  aux = pow((double)reg[s], (double)reg[t]) ;
                  reg[r] = (int)aux ;  break;
    case opMOD :  reg[r] = reg[s] % reg[t] ;  break;
